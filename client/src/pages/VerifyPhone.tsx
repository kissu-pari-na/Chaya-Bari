...
export function VerifyPhone() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setSubmitting(true)
    try {
      const user = await verifyPhone(phone.trim(), code.trim())
  ...
  async function handleResend() {
    ...
    setResending(true)
    try {
      await apiRequest('/auth/resend-phone', { method: 'POST', body: { phone: phone.trim() } })
  ...
  return (
    <div className="auth-page">
      <form className="auth-card" onSubmit={handleSubmit}>
        ...
        <h1>{t('মোবাইল নম্বর নিশ্চিত করুন', 'Confirm your mobile number')}</h1>
